...
export class Get {
    public async posts(req: Request, res: Response): Promise<void> {
        ...
        //error      
        if(cachedPosts.length) { // get from cache before mongodb
            posts = cachedPosts;
            totalPosts = await postCache.getTotalPostsFromCache();
        }else{//get from mongodb
    ...
    public async postsWithImage(req: Request, res: Response): Promise<void> {
        ...
        posts = cachedPost.length ? cachedPost : await postService.getPosts({ imgId: '$ne', gifUrl: '$ne'}, skip, limit, { createdAt: -1});
