...
import { INotificationTemplate } from '@notification/interfaces/notification.interfaces';
import { emailQueue } from '@service/queues/email.queue';
import { notificationTemplate } from '@service/emails/template/notifications/notification-templete';
...
export class Add {
    ...
    public async message(req: Request, res: Response): Promise<void> {
        ...
        if (selectedImage.length) {
            ...
            fileUrl = `https://res.cloudinary.com/dxxifd68q/image/upload/v${result.version}/${result.public_id}`;
        }
        ...
        await messageCache.addChatMessageToCache(messageData, `${conversationObjectId}`);
        //add message to chat queue
        chatQueue.addChatToDb('addChatMessageToDB', messageData);
    ...
    private emitSocketIOEvent(data: IMessageData): void {
        socketIOChatObject.emit('message received', data); //update chat message
        socketIOChatObject.emit('chat list', data); //update chat list
    }
