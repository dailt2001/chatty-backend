...
export class MessageCache extends BaseCache {
    ...
    public async addChatMessageToCache(value: IMessageData, conversationId: string): Promise<void> {
        try {
            if (!this.client.isOpen) {
                await this.client.connect();
            }
        ...
        try {
            ...
            if (parsedReceiver) {
                ...
                for (const item of userMessages) {
                    chatMessages.push(Helpers.parseJson(item));
                }
                return chatMessages;
            } else {
                return [];
