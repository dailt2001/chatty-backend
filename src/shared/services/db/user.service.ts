...
class UserService {
    ...
    public async updatePassword(username: string, hashedPassword: string): Promise<void>{
      await AuthModel.updateOne({username} , { $set: { password: hashedPassword}});
    }
    ...
    public async getUserById(userId: string): Promise<IUserDocument> {
        const users: IUserDocument[] = await UserModel.aggregate([
            { $match: { _id: new mongoose.Types.ObjectId(userId) } }, //findOne userId la string nen ko truyen truc tiep dc vao
    ...
    public async getUserByAuthId(authId: string): Promise<IUserDocument> {
        const users: IUserDocument[] = await UserModel.aggregate([
            ...
            { $lookup: { from: 'Auth', localField: 'authId', foreignField: '_id', as: 'authId' } },
            { $unwind: '$authId' }, // chuyen du lieu tra ve sang object authId
            { $project: this.aggregateProject() }
        ]);
        return users[0];
    }
    ...
    public async getAllUsers(userId: string, skip: number, limit: number): Promise<IUserDocument[]> {
        const users: IUserDocument[] = await UserModel.aggregate([
            ...
            { $sort: { createdAt: -1 } },
            { $lookup: { from: 'Auth', localField: 'authId', foreignField: '_id', as: 'authId' } },
            //Auth Collection authId => _id(user Collection)
            { $unwind: '$authId' },
            { $project: this.aggregateProject() }
        ]);
        return users;
    ...
    public async getTotalUsers(): Promise<number> {
        const count: number = (await UserModel.find({}).countDocuments()) as number;
        return count;
    }
